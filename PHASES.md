...
| **39.F** | Drop `modernc.org/sqlite` from `go.mod` and `go.sum`. The SQLite persistence layer (`internal/persistence`) is deleted along with the world-AI weekly generation artefacts (`worldpacks/frontier/{actions,entities,factions,generation,llm,occupations,rules}.yaml` — v1 sim worldpack config loaded by nothing in v2). | `go mod tidy` drops modernc.org/sqlite + cascade; `grep -rn sqlite .` confirms zero `.go` references; build + vet + full short test sweep green; one stale `persistence/CLI layer` comment in `internal/state/save_test.go` tightened to `JSON SaveGame / save-CLI layer`. | ✅ done |
